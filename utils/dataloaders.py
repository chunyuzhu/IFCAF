...
def img2label_paths(img_paths):
    ...
    # return [x.replace('JPEGImages', 'labels').replace('jpeg', 'txt') for x in img_paths]  # for FLIR Dataset
    return [x.replace('visible', 'labels').replace("infrared", "labels").replace('jpg', 'txt').replace('jpeg', 'txt') for x in img_paths]  # for FLIR Dataset
    # return [x.replace('visible/train', 'labels').replace('visible/test', 'labels').replace('infrared/test', 'labels').replace("infrared/train", "labels").replace('jpg', 'txt').replace('jpeg', 'txt') for x in img_paths]  # for LLVIP Dataset
    # return [x.replace('vi', 'labels').replace('ir', 'labels').replace('.png', '.txt') for x in img_paths]  # for M3FD Dataset
...
def verify_image_label(args):
    ...
    try:
        ...
        if os.path.isfile(lb_file):
            nf = 1  # label found
            with open(lb_file) as f:
                lb = [x.split() for x in f.read().strip().splitlines() if len(x) and x[0] != "3"]  # LiChao Change
...
def create_dataloader_rgb_ir(path1, path2,  imgsz, batch_size, stride, opt, hyp=None, augment=False, cache=False, pad=0.0, rect=False,
                      rank=-1, world_size=1, workers=0, image_weights=False, quad=False, prefix='', sampler=None):
...
class LoadMultiModalImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path_rgb, path_ir, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        try:
            ...
            # img_files_rgb和img_files_ir分别储存了rgb图像和ir图像的绝对路径
        ...
        self.label_files_rgb = img2label_paths(self.img_files_ir)
        cache_rgb_path = (p_rgb if p_rgb.is_file() else Path(self.label_files_rgb[0]).parent).with_suffix('.cache')  # cached labels--
        if cache_rgb_path.is_file():  # 如果生成的路径指向的是一个存在的.cache文件
            cache_rgb, exists_rgb = torch.load(cache_rgb_path), True  # load  ---读取.cache文件中的内容
            if cache_rgb['hash'] != get_hash(self.label_files_rgb + self.img_files_rgb) or 'version' not in cache_rgb:
        ...
        if self.rect:
            ...
            self.shapes_rgb = s_rgb[irect_rgb]
            ...
            try:
                ...
                if os.path.isfile(lb_file):
                    ...
                    with open(lb_file, 'r') as f:
                        ...
                        # ####### filter label ##############
                        l = [x.split() for x in f.read().strip().splitlines() if x[0] != "3"]  # LiChao for FLIRDataset
                        # l = [x.split() for x in f.read().strip().splitlines()]  # Lichao For LLVIP and M3FD
                        #########################################################################################
...
def load_image_rgb_ir(self, index):
    ...
    if (img_rgb is None) and (img_ir is None):  # not cached
        ...
        img_rgb = cv2.imdecode(np.fromfile(path_rgb, dtype=np.uint8), -1)
        img_rgb = cv2.cvtColor(img_rgb, cv2.COLOR_RGB2BGR)
        img_ir = cv2.imdecode(np.fromfile(path_ir, dtype=np.uint8), -1)
        img_ir = cv2.cvtColor(img_ir, cv2.COLOR_RGB2BGR)
